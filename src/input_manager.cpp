...
void InputManager::initialize_input_map() {
	...
	// Avoid duplicate events on re-init (e.g. multiple PlayerControllers or hot-reload)
	bool has_shoot_mouse = false;
	Array events = input_map->action_get_events("shoot");
	for (int i = 0; i < events.size(); i++) {
		Ref<InputEventMouseButton> btn = events[i];
		if (btn.is_valid() && btn->get_button_index() == MOUSE_BUTTON_LEFT) {
			has_shoot_mouse = true;
			break;
		}
	}
	if (!has_shoot_mouse) {
